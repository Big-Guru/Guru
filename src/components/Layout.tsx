...
import { Home, Users, Briefcase, Bell, LogOut, Menu, X, Map } from 'lucide-react';
...
export default function Layout() {
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden bg-slate-50 font-sans antialiased text-slate-800">
      ...
      {/* Sidebar Navigation */}
      <aside className={cn(
        "fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200/80 flex flex-col shrink-0 transition-all duration-300 ease-in-out lg:translate-x-0 h-full",
        sidebarOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"
      )}>
        {/* Brand Header */}
        <div className="h-16 px-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 bg-gradient-to-tr from-blue-600 to-indigo-600 rounded-xl flex items-center justify-center text-white font-extrabold shadow-lg shadow-blue-500/10 tracking-tight text-lg">
              G
            </div>
            <span className="text-slate-900 font-bold text-lg tracking-tight">Guru Workspace</span>
          </div>
          <button className="lg:hidden text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-50" onClick={() => setSidebarOpen(false)}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 py-6 px-4 space-y-6 overflow-y-auto">
          <div>
            <p className="px-3 text-[10px] font-bold uppercase tracking-wider text-slate-400/90 mb-3">Navigation</p>
            <nav className="space-y-1">
              {links.map((link) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Icon className={cn("w-4.5 h-4.5 transition-colors duration-200", 
                        isActive ? "text-blue-600" : "text-slate-400 group-hover:text-slate-600"
                      )} />
                      <span>{link.label}</span>
                    </div>
                    {link.to === '/alerts' && criticalCount > 0 && (
                      <span className="h-5 min-w-5 px-1.5 bg-red-100 text-red-600 text-[10px] font-bold rounded-full flex items-center justify-center animate-pulse">
                        {criticalCount}
                      </span>
                    )}
                  </Link>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/30">
          <div className="flex items-center justify-between p-3 rounded-2xl border border-slate-100 bg-white hover:shadow-md hover:border-blue-100/50 transition-all duration-300 group cursor-pointer" onClick={handleLogout} title="Se déconnecter">
            ...
            <div className="p-1.5 rounded-lg bg-slate-50 group-hover:bg-red-50 text-slate-400 group-hover:text-red-500 transition-colors shrink-0">
